...
export function getEvo(rate: number): Evo {
  ...
  if (rate < 80)
    return { label: "通常", accent: "#3b82f6", glow: "rgba(59,130,246,0.35)" };
...
export function useSlotMachine(): UseSlotMachineReturn {
  ...
  const handleRecord = useCallback(() => {
    ...
    insertLog({
      content: result1.name,
      duration: result2.minutes,
      status: false,
    }).then(({ error }) => {
      if (error) return;
      setSaved(true);
  ...
  const contentNames = useMemo(() => contents.map((c) => c.name), [contents]);
  const durationLabels = useMemo(
    () => durations.map((d) => `${d.minutes}分`),
    [durations],
  );
